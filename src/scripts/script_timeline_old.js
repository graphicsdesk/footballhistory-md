...
const events = [
    {date: new Date('1961-11-18'), label: 'Columbia wins first Ivy League Title', description: 'Columbia wins their first Ivy League title with head coach Aldo Donelli. They go 6-1 in the League with their only loss coming to Princeton.', segment:'start',link: 'https://cdn.pixabay.com/photo/2017/05/11/16/40/emoji-2304720_1280.png'},
    {date: new Date('1983-10-22'), label: 'Start of 44-game losing streak', description: 'Columbia wins their first Ivy League title with head coach Aldo Donelli. They go 6-1 in the League with their only loss coming to Princeton.', segment:'end'},
...
function wrapText(text, width) {
  ...
  var line = [];
  var lineHeight = 1.1; // Adjust line height to your liking
  var lines = [];
  var testLine = '';
  var testWidth = 0;
  ...
  for (var i = 0; i < words.length; i++) {
      ...
      testWidth = text.node().getComputedTextLength();

      // If the line exceeds the width, push the current line and start a new one
      if (testWidth > width) {
  ...
  // Clear the text and add the wrapped lines
  text.text('');
  lines.forEach(function(line, index) {
      text.append('tspan')
          .attr('x', text.attr('x'))
          .attr('dy', index === 0 ? 0 : lineHeight) // Adds vertical spacing
          .text(line);
  });
}
